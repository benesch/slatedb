...
// handler that will print backtraces for all async tasks if the test is
...
pub fn test(tokio_test_attrs: TokenStream, test_fn: TokenStream) -> TokenStream {
    ...
    quote! {
      ...
      async fn #test_fn_name() #test_fn_ret {
        ...
        let sigterm_handler_task = tokio::spawn(async move {
          signal.recv().await;
          eprintln!(
            "backtraces for running async tasks:\n{}",
